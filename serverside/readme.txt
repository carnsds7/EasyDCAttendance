This is the server side of the facial recognition attendance tracker. The server listens for
a number of clients (by default 5). Then creates a thread for each client to process signing them in or out. The ipaddress of the server is specified in the server_config.json file. 
...
Data from the users are kept in the specified database file (as well as most recent activity stored in the google sheet). When a client closes the connection, the server updates the
